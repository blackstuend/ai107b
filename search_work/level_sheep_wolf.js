...
function search(array,level) { //廣度優先
    var thing = array.shift() //取第一個
    if (thing.thing.toString() == correct_thing.toString())
    {
        correct_move.push(thing.thing)
        return thing.last_thing
    }
    //=========find neighbor=====
    find_neighbor(thing)
    //========find neighbor======
    var result = search(arr,level + 1) //result was last thing
    if(level == 1){
        console.log([0,0,0,0])
        for(let i=correct_move.length-1;i>=0;i--){
            console.log(correct_move[i])
        }
    }
    if (result.toString() == thing.thing.toString()) {
        correct_move.push(result)
        return thing.last_thing
    } else
        return result
}
